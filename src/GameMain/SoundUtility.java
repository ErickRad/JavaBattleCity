...
public class SoundUtility {
    ...
    public static void BulletHitBrick() {
        if (initialized) { bulletBrickSE.loop(1); } else { initialize(); bulletBrickSE.loop(1); }
    }

    public static void BulletHitTank() {
        if (initialized) { bulletTankSE.loop(1); } else { initialize(); bulletTankSE.loop(1); }
    }

    public static void fireSound() {
        if (initialized) { fireSE.loop(1); } else { initialize(); fireSE.loop(1); }
    }

    public static void explosion1() {
        if (initialized) { explosion1SE.loop(1); } else { initialize(); explosion1SE.loop(1); }
    }

    public static void explosion2() {
        if (initialized) { explosion2SE.loop(1); } else { initialize(); explosion2SE.loop(1); }
    }

    public static void startStage() {
        if (initialized) { startStageSE.loop(1); } else { initialize(); startStageSE.loop(1); }
    }

    public static void pause() {
        if (initialized) { pauseSE.loop(1); } else { System.out.println("pause"); initialize(); pauseSE.loop(1); }
    }

    public static void powerupPick() {
        if (initialized) { powerupPickSE.loop(1); } else { initialize(); powerupPickSE.loop(1); }
    }

    public static void gameOver() {
        if (initialized) { gameoverSE.loop(1); } else { initialize(); gameoverSE.loop(1); }
    }

    public static void statistics() {
        if (initialized) { statisticsSE.loop(1); } else { initialize(); statisticsSE.loop(1); }
    }
}
